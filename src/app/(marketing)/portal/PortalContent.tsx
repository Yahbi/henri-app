...
/* Territory matching downstream keys off a plain 5-digit ZIP, so ZIP+4,
 * city names and typos have to be corrected at the input rather than
 * carried into the intake flow and failing silently there. */
const ZIP5 = /^\d{5}$/;
const ZIP_ERROR = "Enter a 5-digit US ZIP code, like 90278.";

export interface PortalContentProps {
  /** "30+" — measured coverage, rounded DOWN. From getLandingStats(). */
  activeStatesLabel: string;
}

export function PortalContent({ activeStatesLabel }: PortalContentProps) {
  ...
  /* Validates the field and publishes the inline error as a side effect.
   * `allowEmpty` distinguishes the two entry points: submitting the hero
   * form with nothing typed is an error, whereas clicking a trade card
   * before touching the ZIP field is a legitimate path (the chat asks for
   * the address at Step 1). A non-empty but malformed value is an error in
   * both cases — the user typed something and deserves to know it didn't
   * take, rather than have it trimmed, padded, or dropped behind their
   * back. */
  const readZip = useCallback(
    (allowEmpty: boolean): { ok: true; zip: string } | { ok: false } => {
      ...
      if (!trimmed) {
        ...
        setZipError(ZIP_ERROR);
        return { ok: false };
      }
      if (!ZIP5.test(trimmed)) {
        setZipError(ZIP_ERROR);
        return { ok: false };
      }
      setZipError(null);
      return { ok: true, zip: trimmed };
    },
    [zipInput]
  );
  ...
  const handleTradeClick = useCallback(
    (trade: string) => {
       ...
       * forced a double-ask. */
      const result = readZip(true);
      if (!result.ok) {
        /* The error message lives up in the hero, well off-screen from the
         * trade grid — move focus to it so the correction is discoverable
         * instead of a click that appears to do nothing. */
        zipFieldRef.current?.focus();
        return;
      }
      openChat(result.zip, trade);
    },
    [readZip, openChat]
  );
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-background pt-12 pb-20 lg:pt-20 lg:pb-28">
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          {/* ZIP form. The field keeps exactly what was typed — validation
              happens on submit and reports back, rather than rewriting the
              value under the user. */}
          <div className="mx-auto mt-10 max-w-md">
            <form onSubmit={handleHeroSubmit} className="flex gap-3" noValidate>
              <input
                ref={zipFieldRef}
                type="text"
                inputMode="numeric"
                ...
                onChange={(e) => {
                  ...
                  /* Clear on edit so the message tracks the current value,
                ...
                aria-describedby={zipError ? "portal-zip-error" : undefined}
                className={cn(
                  "flex-1 rounded-lg border bg-card px-4 py-3 text-sm text-foreground shadow-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2",
                  zipError
      ...
      <section className="border-y border-border bg-card py-12">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-border bg-background p-6 text-center">
              {/* Measured, not authored — see page.tsx. This was a hardcoded
                  "30+" that could only be corrected by a human noticing it. */}
